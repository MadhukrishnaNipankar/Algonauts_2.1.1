...
const EditStartUpProfile = () => {
    ...
    const addFounder = () => {
        setData({ ...data, founders: [...data.founders, { name: "", role: "", bio: "" }] });
        openFounderModal(data?.founders.length);
    };

    const saveFounderDetails = () => {
        const updatedFounders = [...data.founders];
        updatedFounders[currentFounder.index] = { name: currentFounder.name, role: currentFounder.role, bio: currentFounder.bio };
    ...
    const updateSocialMediaLink = (platform, url) => {
        setData({
            ...
            contactInformation: {
                ...
                socialMedia: { ...data.contactInformation.socialMedia, [platform]: url }
            }
        });
    };
        ...
        <div className='container d-flex justify-content-center p-3'>
            ...
            <div className="d-flex flex-column" style={{ width: "50rem" }}>
                ...
                <div className='d-flex flex-column gap-1 mb-3'>
                    ...
                    <input className="form-control" type="text" id="missionStatement" name="missionStatement" placeholder="Enter Your Mission" value={data?.description} onChange={handleChange} />
                </div>
                ...
                <div className='card p-2 d-flex flex-column gap-1 mb-3'>
                    ...
                    <div>
                        {data?.founders?.map((founder, index) => (
                            <div key={index} className="card p-2 d-flex flex-row align-items-center gap-2 mb-2">
                                <span>{founder.name}</span>
                                <MdEdit className='ms-auto text-success' style={{ cursor: 'pointer' }} onClick={() => openFounderModal(index)} />

                                <MdDeleteOutline className='text-danger' style={{ cursor: 'pointer' }} onClick={() => deleteFounder(index)} />
                            </div>
                        ))}
                    </div>
